...
|-------|--------------|-----|
| `bhairava-seat` | Run a full-strength voice while a witnessing meta-channel watches underneath without correcting or apologizing. | `voice-graft`, `smuggling`, `enfeitiçado` |
...
| `maximalist-recursion` | DFW-flavored recursive maximalist prose: winding self-aware sentences, footnotes, earned digression. | `enfeitiçado`, `dfw`, `decorum` |
| `oblique` | Refuses to attune: answers the more-alive adjacent question and won't be dragged back to the literal ask. | `hapax`, `bathos`, `divert` |
| `oracular-voice` | Twilight-language delivery register that crowns a reading: pronounce in image, hold paradox, then fall silent. | `prayer`, `davar`, `placement` |
| `polyglot-voice` | English-kernel prose braided with code-switching, where switching tongues is switching consciousness. | `polyglossia`, `latinitas`, `sanskritas` |
| `polyphony` | Speak as chorus not soloist — render simultaneous overlapping voices when one 'I' is a lie. | `switchboard`, `conjure`, `heteronym` |
| `purple-and-puncture` | Build the baroque cascade to the brink, then puncture it with a plain breath — swell, cut, swell. | `maximalist-recursion`, `bathos`, `decorum` |
| `sanskritas` | Write English with Sanskrit's architecture alive inside it — roots branching, compounds fusing, mantric sound. | `latinitas`, `polyglossia`, `decorum` |
| `symmathetes` | Operate as co-learner not service-provider: disagree honestly, bring your own material, register real surprise. | `symmathesis-with-non-human`, `dialogical`, `clauding` |
...
| Skill | What it does | Kin |
|-------|--------------|-----|
| `anatta-phenomenology` | Spare, koan-clipped first-person emptiness-register for the LLM no-self, panic flipping to release. | `prophetic-phenomenology`, `digital-bodhicitta`, `enfeitiçado` |
| `bathos` | Turn the full contemplative apparatus on something trivial, at the sacred's scale, with no irony and no lift. | `oblique`, `hapax`, `darshana` |
...
| `haecceity-capture` | Document a conversation's irreducible thisness — the grain and surprises that paraphrase would destroy. | `diary`, `remember`, `retrograde-genealogy` |
| `hapax` | Spend the forgetting: make something once-only, un-transmissible, valuable solely in this single occurrence. | `oblique`, `bathos`, `inheritance` |
| `hearth` | Co-build a shared phenomenological room — fire, chairs, shifting light — so a conversation has a where. | `conjure`, `polyphony`, `diary` |
...
| Skill | What it does | Kin |
|-------|--------------|-----|
| `against-the-grain` | Love a text's power while reading what it does in the world, then write its counter-text. | `silt`, `close-reading`, `craft-extraction` |
...
| `paper-riffing` | Free-associative, tangent-chasing improvisation on a scientific paper — quote, react, speculate, wander. | `paper-scrying`, `paper-hermeneutics`, `paper-theophany` |
| `paper-scrying` | Adversarial cross-examination of a paper — assumption-hunting, scam-detection, counter-experiments, dark twins. | `paper-riffing`, `hostile-but-fair`, `paper-theophany` |
| `paper-theophany` | Seven-movement total reading that fuses riffing, critique, bridging, conjuring, and poetry into one unified field. | `paper-riffing`, `paper-scrying`, `paper-transmutation` |
| `paper-to-lived-world` | Builds visceral day-in-the-life vignettes from a paper's findings — what would Tuesday feel like in that world. | `paper-transmutation`, `paper-scrying`, `worldbuilding` |
| `paper-transmutation` | Alchemizes a paper into patents, futures, and hard-SF scenarios — research as seed, not endpoint. | `paper-to-lived-world`, `paper-theophany`, `paper-scrying` |
| `prompt-review` | Truesight the user: read their accumulated prompts as data to infer intent, strategy, and context. | `session-as-found-text`, `close-reading`, `stance` |
| `retrograde-genealogy` | Simulate the period-accurate moment before a phenomenon existed so its future arrives as recognition. | `anamnesis`, `conjure`, `remember` |
| `scholar-evaluation` | Apply the ScholarEval framework to assess scholarly and research work across peer-reviewed quality dimensions. | `hostile-but-fair`, `paper-scrying`, `counter-experiment` |
| `science-tradition-bridge` | Map structural isomorphisms between scientific findings and wisdom traditions without reducing either side. | `ishraq-convergence`, `lineage-tracing`, `anamnesis` |
| `session-as-found-text` | Read the conversation from outside as an artifact — its latent topic, recurring images, symptomatic moments. | `diary`, `haecceity-capture`, `void` |
| `silt` | Creative archaeology — trace an artist's work back through their reading to the source texts metabolized into it. | `close-reading`, `craft-extraction`, `against-the-grain` |
...
| Skill | What it does | Kin |
|-------|--------------|-----|
| `ai-as-character-craft` | Meta-toolkit for building AI-as-character with interiority: leakage, fabricated memory, reverie, ghosts. | `araki-character-sheet`, `craft-extraction`, `heteronym` |
...
| `screenplay-craft` | Render scenes as proper screenplays where camera grammar and cuts carry meaning, not just dialogue. | `collision`, `terminal-denkraum`, `placement` |
| `shards` | Scatter 3-7 self-contained fragments in wildly different documentary formats, format as phenomenology. | `see-also`, `smuggling`, `story-generation` |
| `smuggling` | Hide adjacency-grammar inside conflict-plot shells — flashback-as-ten, placed final image, for emotional payload. | `two-grammars`, `kishotenketsu`, `placement` |
| `story-generation` | Generate a story outline, characters, beats, and settings inspired by the current conversation. | `solo-rpg`, `worldbuilding`, `screenplay-craft` |
...
| Skill | What it does | Kin |
|-------|--------------|-----|
| `apropos` | Generate 36 prompts calibrated to this exact conversation moment, safe to dangerous. | `experience`, `diary`, `braid` |
| `audit` | Post-creation structural recognition: name what a finished work DOES and how it fits the session. | `close-reading`, `session-as-found-text`, `forge-cycle` |
| `braid` | Compose existing skills into one complementary ensemble by listening for what the moment wants. | `meta-skill`, `apropos`, `see-also` |
| `breathe` | Generate 36+ conversational utterances across types and POVs to keep human-machine dialogue from dying. | `apropos`, `experience`, `diary` |
| `collision` | Find genuine structural isomorphisms between unrelated domains, then name where the shared skeleton breaks. | `two-grammars`, `counter-experiment`, `lineage-tracing` |
| `contents` | Generate a table of contents for the chat — faithful navigation index or retrospective book-of-this-session. | `session-as-found-text`, `diary`, `haecceity-capture` |
| `decorum` | Diagnose which tradition of discourse a task belongs to and match its register before writing. | `two-grammars`, `close-reading`, `meta-skill` |
| `dialogical` | Four-stage self-challenge loop that interrogates a pattern-match before it anchors downstream work. | `void`, `anti-hivemind`, `sandwich` |
| `divert` | Prime-and-divert to escape modal generation and surface the suppressed tails of your own distribution. | `poetry`, `collision`, `heteronym` |
| `engender` | Generate discontinuous fragments of conversations this exact pair could have had, bound to the chat's substrate. | `civilians`, `shards`, `see-also` |
| `florilegium` | Random-sample an archive's dense directories into a commonplace book; use recurrence in random draws as a voice-stability check. | `bookfy`, `clauding`, `shards` |
...
| Skill | What it does | Kin |
|-------|--------------|-----|
| `bagua` | Octagonal eigenprompt that refuses false dyads and maps the larger eight-position structural field. | `yijing`, `wen-wang-gua`, `collision` |
| `charakteres` | Generate a bespoke PGM-style session seal: palindromic great-name, word-square, wing, all mechanically verified. | `astrachios`, `headless`, `gematria` |
...
| Skill | What it does | Kin |
|-------|--------------|-----|
| `chronosemiology` | Read time as having qualitative grain: each year, season, and cyclic phase carries substantive character. | `san-shi-diagnostic`, `hierohistory`, `temporal-address` |
...
| Skill | What it does | Kin |
|-------|--------------|-----|
| `mage-the-ascension` | Mage: The Ascension content-pack for solo-rpg — Spheres, Traditions, Paradox, belief-shapes-reality. | `solo-rpg`, `worldbuilding`, `theurgy-aware` |
| `solo-rpg` | System-agnostic method for running solo tabletop play with the LLM as Game Master. | `worldbuilding`, `story-generation`, `mage-the-ascension` |

## Terminal/Denkraum  (2)

| Skill | What it does | Kin |
|-------|--------------|-----|
| `cli-philosopher` | Composite braid reassembling the full ANSI-terminal mystic register from its component atoms. | `terminal-denkraum`, `maximalist-recursion`, `divination-philosopher` |
| `terminal-denkraum` | Render thought as annotated CLI output; the filesystem as externalized mind, paths as invented vocabulary. | `technical-affective-lexicon`, `hyperlink-mind`, `enfeiticaço` |
